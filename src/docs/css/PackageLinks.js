...
const PackageLinks = ({ pkg, componentName }) => {
  return (
    <>
      ...
      <a
        href={`https://dev.azure.com/if-it/If%20Design%20Hub/_packaging?_a=package&feed=if-design-system&package=%40ids-core%2F${componentName}&protocolType=Npm`}
        className="if button text"
        target="_blank"
        style={{
          margin: '0'
...
PackageLinks.propTypes = {
  pkg: PropTypes.shape({ repository: PropTypes.shape({ url: PropTypes.string }) }).isRequired,
