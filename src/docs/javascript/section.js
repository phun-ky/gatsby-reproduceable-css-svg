...
const Section = ({ edges, edge, pkg /*wide*/ }) => {
  ...
  const _component_package = getComponentPackage(componentName, pkg, 'js');
  const _is_first_section_of_component = !!componentName && _component_package && order === 1;
  const _is_js_only_package = [
    '/components/cards/card/js',
    '/components/overlay/dialog/js',
    ...
    <div className={_section_class_names} id={_id}>
      <div className="if documentation-container">
        <LinkBackToGuidelines
          title={title}
          show={_is_first_section_of_component && !_is_js_only_package}
          to={customPath.replace('/js', '')}
        />
        <Title
        ...
        {order === 1 && <PackageInstall scope="ids-js" name={componentName} pkg={_component_package} />}
        {_documentation_body}
